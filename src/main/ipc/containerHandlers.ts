...
export function registerContainerHandlers(docker: Docker): void {
  // List containers
  ipcMain.handle(IPC.CONTAINER_LIST, async (_event, { all }: { all: boolean }): Promise<IpcResponse<ContainerInfo[]>> => {
    try {
  ...
  // Start a container
  ipcMain.handle(IPC.CONTAINER_START, async (_event, { id }: { id: string }): Promise<IpcResponse<void>> => {
    try {
  ...
  // Stop a container
  ipcMain.handle(IPC.CONTAINER_STOP, async (_event, { id }: { id: string }): Promise<IpcResponse<void>> => {
    try {
      const container = docker.getContainer(id);
  ...
  // Remove a container
  ipcMain.handle(IPC.CONTAINER_REMOVE, async (_event, { id, force }: { id: string; force: boolean }): Promise<IpcResponse<void>> => {
    try {
      const container = docker.getContainer(id);
  ...
  // Inspect a container
  ipcMain.handle(IPC.CONTAINER_INSPECT, async (_event, { id }: { id: string }): Promise<IpcResponse<ContainerInfo>> => {
    try {
      const container = docker.getContainer(id);
