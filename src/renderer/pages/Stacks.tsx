  ...
  return (
    <main className={styles.page} aria-label="Gerenciador de stacks">
      ...
      <div className={styles.toolbar}>
        ...
        <Button label="Atualizar" variant="ghost" onClick={fetchStacks} data-testid="refresh-stacks" />
      </div>

      {error && <div role="alert" className={styles.error}>{error}</div>}

      {loading ? (
        <div role="status" className={styles.loading} aria-label="Carregando stacks...">Carregando...</div>
      ) : stacks.length === 0 ? (
        <div className={styles.empty}>
          <p>Nenhuma stack Compose encontrada.</p>
          <p>Stacks aparecem aqui quando containers são iniciados com <code>docker compose up</code>.</p>
        </div>
      ) : (
            ...
            >
              <div className={styles.stackHeader}>
                <div>
                  ...
                  {stack.configFile && (
                    <p className={styles.configFile}>{stack.configFile}</p>
                  )}
                </div>
                <StatusBadge status={statusMap[stack.status]} data-testid={`stack-status-${stack.name}`} />
              </div>
              <ul className={styles.serviceList} aria-label="Serviços da stack">
                {stack.services.map((svc) => (
                  <li key={svc} className={styles.serviceItem}>{svc}</li>
                ))}
              </ul>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button label="Parar stack" variant="danger" onClick={handleDown} data-testid="confirm-down" />
          </>
        }
      >
        <p>Tem certeza que deseja parar a stack <strong>{downTarget?.name}</strong>?</p>
